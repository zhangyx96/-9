...
for k=1:m
    for j=1:n
        pix=hall_t(k,j);
        pix_bin=dec2bin(abs(pix));
        pix_bin(end)=num2str(info(k,j));
        hall_t(k,j)=bin2dec(pix_bin);
    end
end
for k=1:8:m-7
for j=1:8:n-7
block_i=hall_t(k:k+7,j:j+7);
block_zz=ZigZag(block_i);
...
pe_ecode=zeros(m,n);
pe_ecode_t=zeros(m,n);
for k=1:8:m-7
for j=1:8:n-7
bs=(k-1)/8*n/8+(j+7)/8;  %块个数
block_izz=iZigZag(c(:,bs));
pe_ecode_t(k:k+7,j:j+7)=block_izz;
end
end
%信息提取
info_decrypted=[];
...
for k=1:8:m-7
for j=1:8:n-7
block_izz=pe_ecode_t(k:k+7,j:j+7);
...
ise=num2str(sum(sum(info_decrypted==info))/20160);
